...
export default function Exchange() {
  ...
  const fetchUserBalances = async () => {
    try {
      ...
      const response = await fetch(`https://currencyexchange-ll7e.onrender.com/api/v1/students/${userId}`);
  ...
  const finalizeExchange = async () => {
    try {
      ...
      const response = await fetch(
        `https://currencyexchange-ll7e.onrender.com/api/v1/students/${userId}/exchange?fromCurrency=${fromCurrency}&toCurrency=${toCurrency}&fromAmount=${fromAmountValue}&toAmount=${toAmountValue}`,
        {
          method: "POST",
