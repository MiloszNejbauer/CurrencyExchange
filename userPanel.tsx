...
export default function UserPanel() {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://currencyexchange-ll7e.onrender.com/api/v1/students/${userId}`
      );
  ...
  const handleAddBalance = async () => {
    try {
      ...
      const response = await fetch(
        `https://currencyexchange-ll7e.onrender.com/api/v1/students/${userId}/balance?currency=PLN&amount=${amount}`,
        {
          method: "PATCH",
